...
public class MinIOServiceImpl implements IOssService {
     ...
     */
    @Override
    public void deleteModelFolder(String modelFolder) {
        try {
            // 检查存储桶是否存在
            boolean isExist = client.bucketExists(BucketExistsArgs.builder().bucket(OssConfiguration.modelsBucket).build());
            if (!isExist) {
//                System.out.println("Bucket does not exist: " + OssConfiguration.modelsBucket);
                return;
            }
            // 删除文件夹及其内容
            deleteFolder(modelFolder);
//            System.out.println("Folder '" + modelFolder + "' and its contents deleted successfully.");
    ...
    private  void deleteFolder( String folderPath)
        ...
        Iterable<Result<Item>> results = client.listObjects(
                ListObjectsArgs.builder()
                        .bucket(OssConfiguration.modelsBucket)
                        .prefix(folderPath)
                        .recursive(true) // 递归列出所有子文件夹和文件
                        .build());
