...
import com.dji.sample.df.manageDf.model.enums.UserTypeEnum;
...
@Service
@Transactional
public class UserServiceImplDf implements IUserServiceDf {

    @Autowired
    private IUserMapperDf mapper;
    ...
    public Boolean updateUser(String workspaceId, String userId, UserListDTO user) {
        ...
        } else if (user.getUserType().equals("Pilot")) {
...
//添加一个成员
    ...
    @Override
    public void addUser(String workspaceId, UserListDTO user) {
        UserEntity userEntity = new UserEntity();
        userEntity.setUserId(UUID.randomUUID().toString()); // 设置用户ID，可以使用UUID生成唯一ID
        userEntity.setUsername(user.getUsername());
        userEntity.setPassword(user.getMqttPassword());
...
//根据工作空间和用户id删除
    ...
    //模糊查询用户
    @Override
    public PaginationData<UserListDTO>getWorkspaceUserByname(long page, long pageSize, String workspaceId, String userName) {
        Page<UserEntity> userEntityPage = mapper.selectPage(
                new Page<>(page, pageSize),
                new LambdaQueryWrapper<UserEntity>().eq(UserEntity::getWorkspaceId, workspaceId).like(UserEntity::getUsername,userName));
