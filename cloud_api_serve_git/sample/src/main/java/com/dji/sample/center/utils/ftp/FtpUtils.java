...
public class FtpUtils {
    ...
    private FtpsConfig ftpsConfig = AppContext.getBean(FtpsConfig.class);
    ...
    public Boolean uploadFileToCenterNormal(File localFile, String destDir, String destFileName) {
        ftpsCommonConfig.setFtpIp(centerFtpsNormalConfig.getFtpIp());
        ftpsCommonConfig.setFtpPort(centerFtpsNormalConfig.getFtpPort());
        ftpsCommonConfig.setImplicit(centerFtpsNormalConfig.getImplicit());
        ftpsCommonConfig.setUserName(centerFtpsNormalConfig.getUserName());
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (ftpsHelper.isLogin()) {
                try {
                    result = ftpsHelper.localFileUpload(localFile, destDir, destFileName);
                    if (!result) {
                        ftpsPool.destory(ftpsHelper);
                        ftpsPool.remove(ftpsHelper);
                    } else {
    ...
    public Boolean uploadToFtpNormal(String localFile, String destDir, String destFileName) {
        ftpsCommonConfig.setFtpIp(ftpsConfig.getFtpIp());
        ftpsCommonConfig.setFtpPort(ftpsConfig.getFtpPort());
        ftpsCommonConfig.setImplicit(ftpsConfig.getImplicit());
        ftpsCommonConfig.setUserName(ftpsConfig.getUserName());
        ftpsCommonConfig.setPassword(ftpsConfig.getPassword());
