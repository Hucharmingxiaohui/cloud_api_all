package com.dji.sample.df.waylineDf.service.impl;
...
import com.dji.sample.df.waylineDf.model.dto.KmzFileProperties;
import com.dji.sample.df.waylineDf.model.dto.WaylineFileDTO;
...
@Service
@Transactional
public class WaylineFileServiceImplDf implements IWaylineFileServiceDf {

    @Autowired
    private IWaylineFileMapperDf mapper;
    ...
    @Override
    public PaginationData<GetWaylineListResponse> getWaylinesByName(String workspaceId,String name, GetWaylineListRequest param) {
        // Paging Query
        Page<WaylineFileEntity> page = mapper.selectPage(
                new Page<WaylineFileEntity>(param.getPage(), param.getPageSize()),
                new LambdaQueryWrapper<WaylineFileEntity>()
                        .eq(WaylineFileEntity::getWorkspaceId, workspaceId)
                        .eq(Objects.nonNull(param.getFavorited()), WaylineFileEntity::getFavorited, param.getFavorited())
                        .like(WaylineFileEntity::getName,name)
                        .and(param.getTemplateType() != null, wrapper ->  {
                            for (WaylineTypeEnum type : param.getTemplateType()) {
                                wrapper.like(WaylineFileEntity::getTemplateTypes, type.getValue()).or();
                            }
