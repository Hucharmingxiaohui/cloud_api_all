    ...
    public IntegrationFlow statusRouterFlow() {
        return IntegrationFlows
                ...
                .<TopicStatusRequest<UpdateTopo>, Boolean>route(
                        response ->
                                Optional.ofNullable(response.getData()).map(UpdateTopo::getSubDevices).map(CollectionUtils::isEmpty).orElse(true),
                        mapping ->
                                mapping.channelMapping(true, ChannelName.INBOUND_STATUS_OFFLINE)
