...
//        RouteFileUtils.buildKmz("航线kmz文件_1", kmlParams);
//    }
    ...
    public void buildKmz(UavRouteReq uavRouteReq) {
        ...
        String routeName = uavRouteReq.getRouteName(); // 假设 JSON 中有这个字段  //xtj
        if (routeName == null || routeName.trim().isEmpty()) {
            routeName = "默认航线";
        }

        String fileName = routeName; // 拼接最终文件名 xtj
        RouteFileUtils.buildKmz(fileName, kmlParams);
    }

    @Override
    public String buildKmzAndReturnPath(UavRouteReq uavRouteReq) {//8.14
        KmlParams kmlParams = new KmlParams();
        BeanUtils.copyProperties(uavRouteReq, kmlParams);
        kmlParams.setRoutePointList(BeanUtil.copyToList(uavRouteReq.getRoutePointList(), RoutePointInfo.class));

        String routeName = uavRouteReq.getRouteName();
        if (routeName == null || routeName.trim().isEmpty()) {
            routeName = "默认航线";
        }
        String fileName = routeName;

        // RouteFileUtils 会返回完整本地路径
        return RouteFileUtils.buildKmz(fileName, kmlParams);
    }
