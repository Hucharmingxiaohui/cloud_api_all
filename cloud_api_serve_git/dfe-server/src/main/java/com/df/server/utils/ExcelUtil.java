...
import com.df.framework.annotation.Excel.ColumnType;
import com.df.framework.annotation.Excel.Type;
import com.df.framework.annotation.Excels;
...
import com.df.framework.utils.ImageUtils;
import com.df.framework.utils.ReflectUtils;
import com.df.framework.utils.file.FileTypeUtils;
...
public class ExcelUtil<T> {
                    ...
                    if (String.class == fieldType) {
                        ...
                        if (StringUtils.endsWith(s, ".0")) {
                            ...
                            if (StringUtils.isNotEmpty(dateFormat)) {
                                val = DateUtils.parseDateToStr((Date) val, dateFormat);
                            } else {
     ...
     * @return 结果
     */
    public FileDownLoadVO exportExcel(List<T> list, String sheetName) {
     ...
     * @return 结果
     */
    public FileDownLoadVO importTemplateExcel(String sheetName) {
     ...
     * @return 结果
     */
    public FileDownLoadVO exportExcel() {
        OutputStream out = null;
        try {
            // 取出一共有多少个sheet.
            double sheetNo = Math.ceil(1.0 * list.size() / sheetSize);
            ...
            out = Files.newOutputStream(Paths.get(absoluteFile));
            wb.write(out);
            FileDownLoadVO vo = new FileDownLoadVO();
            vo.setFileFullPath(absoluteFile);
            return vo;
        } catch (Exception e) {
            log.error("导出Excel异常{}", e.getMessage());
            throw new FastException("导出Excel失败，请联系网站管理员！");
        } finally {
            if (wb != null) {
                try {
                    wb.close();
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column) {
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                String readConvertUtilserExp = attr.readConverterExp();
                String separator = attr.separator();
                String dictType = attr.dictType();
                if (StringUtils.isNotEmpty(dateFormat) && value != null) {
                    cell.setCellValue(DateUtils.parseDateToStr((Date) value, dateFormat));
                } else if (StringUtils.isNotEmpty(readConvertUtilserExp) && value != null) {
                    cell.setCellValue(convertByExp(ConvertUtils.toStr(value), readConvertUtilserExp, separator));
    ...
    public String getAbsoluteFile(String filename) {
        FileConfig fileConfig = AppContext.getBean(FileConfig.class);
        String downloadPath = fileConfig.getFileSavePath() + "/download/" + filename;
        File desc = new File(downloadPath);
        if (!desc.getParentFile().exists()) {
            boolean mkdirs = desc.getParentFile().mkdirs();
            if (!mkdirs) {
                throw new FastException("创建目录失败");
            }
        }
        return downloadPath;
    }
    ...
    boolean isNeed(Field field) {
        boolean need = false;

        if (StringUtils.isEmpty(this.selected)) {
            return true;
        }
    ...
    private void createExcelField() {
        ...
        for (Field field : tempFields) {
            boolean need = this.isNeed(field);
            if (!need) {
                continue;
            }
