...
import LivestreamDock from '/@/components/livestream-dock.vue'
...
// import iconUrl from '/@/assets/icons/poi-marker-default.png'
...
export default defineComponent({
  ...
  setup () {
    ...
    const root = getRoot()
    const showLive = ref<boolean>(true)
   ...
   * */
    ...
    onMounted(() => {
      const store = useMyStore()
      const app = getApp()
      // console.log('设备的所有信息22222222222222222222222222222222222222')
      useGMapManageHook.globalPropertiesConfig(app)
      setTimeout(() => {
        // showWayline()
        // console.log('设备的所有信息', store?.state.deviceState)
      }, 500)
      // setTimeout(() => {
      //   removeAllWayline()
      // }, 5000)
      // deviceTsaUpdateHook.initMarker1(2, 'sss', 'abcvd', 121.3574, 37.5419)
      // getDeviceMarker(store.state.deviceState)
    })
    onUnmounted(() => {
      removeAllWayline()
...
.radio-custom1{
  height: 37px;
  text-align: center;
  background-color: #022E7B;
  border: 2px solid #02639F !important;
}
.g-map-wrapper {
  height: 100%;
  width: 100%;
