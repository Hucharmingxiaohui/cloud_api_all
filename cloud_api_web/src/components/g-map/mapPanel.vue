<template>
    <div class="g-map-wrapper">
      ...
      <!-- 飞机OSD -->
      <div v-if="osdVisible && osdVisible.visible && !osdVisible.is_dock" v-drag-window class="osd-panel fz12">
...
export default defineComponent({
  ...
  setup () {
    ...
    onMounted(() => {
      ...
      //   removeAllWayline()
      // }, 5000)
      // deviceTsaUpdateHook.initMarker1(2, 'sss', 'abcvd', 121.3574, 37.5419)
    })
    onUnmounted(() => {
      removeAllWayline()
    })
    ...
    function showWayline (lineArr: string[]) {
      ...
      // console.log('地图坐标', convertedLineArr)
      // 检查转换后的坐标是否有效
      ...
      const baseFontSize = 12 // 基础字体大
      const circleColor = 'rgba(0, 123, 255, 0.5)' // 半透明蓝色
      ...
      convertedLineArr.forEach((position, index) => {
        ...
        //   }),
        //   offset: new AMap.Pixel(-34, -18) // 偏移量
        // })
        const circle = new AMap.Circle({
          ...
          fillOpacity: 0.5, // 半透明
          strokeColor: circleColor, // 边框颜色
          strokeWeight: 4 // 边框宽度
        })
        root.$map.add(circle)
        circle_markers.push(circle)
        const text = new AMap.Text({
          text: index.toString(), // 当前循环的索引值
          position: position,
          offset: new AMap.Pixel(-baseFontSize / 2, -baseFontSize / 5 * 4), // 初始偏移量，后续会调整
          style: {
            'font-size': `${baseFontSize}px`, // 初始字体大小
            color: circleColor, // 字体颜色
        ...
        // root.$map.add(marker)
        root.$map.add(text)
        markers.push(text) // 将标注添加到数组中
      })
      root.$map.on('zoomchange', () => {
        const zoom = root.$map.getZoom()
        const newFontSize = baseFontSize * Math.pow(1.2, zoom - 16) // 根据缩放级别调整字体大小
        markers.forEach(marker => {
          marker.setStyle({ 'font-size': `${newFontSize}px` })
          const position = marker.getPosition()
          marker.setPosition(position) // 更新位置以重新应用偏移
          // 更新偏移量以保持文字居中
          const newOffset = new AMap.Pixel(-newFontSize / 2, -newFontSize / 5 * 4) // 更新偏移量
          marker.setPosition(position) // 重新设置位置
          marker.setOffset(newOffset) // 设置新的偏移量
        })
      })
      // 使地图适应所有标注
    ...
    function removeAllWayline () {
      ...
      circle_markers.forEach(circle => {
        root.$map.remove(circle)
      })
      circle_markers.length = 0 // 清空标注数组
    }
