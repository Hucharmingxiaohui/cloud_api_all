      ...
      <div class="left " :style="{ width: leftWidth + 'px' }">
        ...
        <div class="box1">
            <div class="operation">
                ...
                <div class="item"><el-button class="btn" :disabled="isSolarRouteSubmitting" @click="handlegenerate" >生成航线</el-button></div>
...
<div>
  ...
  <button class="confirm-button" :disabled="isSolarRouteSubmitting" @click="handlegenerate">生成航线</button>
...
const solarPreview = ref(null)
const solarPreviewVersion = ref(0)
const isSolarRouteSubmitting = ref(false)
let solarPreviewTimer = null
...
function bindSolarPreviewHmrEvents () {
  ...
  import.meta.hot.on('solar3d-preview-updated', ({ payload, version }) => {
    applySolarPreviewPayload(payload, version)
  })
}

function applySolarPreviewPayload (payload, version = Date.now()) {
  if (!payload?.route_draft_id || !Array.isArray(payload.waypoints)) return false

  solarPreviewVersion.value = version
  saveSolar3DPreviewPayload(payload)
  loadSolarPreview(payload)
  return true
}

function startSolarPreviewPolling () {
  if (import.meta.env.MODE !== 'development') return

  solarPreviewTimer = window.setInterval(async () => {
    try {
      const { payload, version } = await fetchDevSolar3DPreview()
      if (!payload || !version || version === solarPreviewVersion.value) return

      applySolarPreviewPayload(payload, version)
    } catch (error) {
      console.warn('轮询本地临时光伏三维预览报文失败:', error)
    }
  }, 1000)
}
...
onBeforeUnmount(() => {
  ...
  if (solarPreviewTimer) {
    clearInterval(solarPreviewTimer)
    solarPreviewTimer = null
  }
  document.removeEventListener('contextmenu', handleRightClick)
  window.removeEventListener('message', handleSolarPreviewMessage)
  if (window.receiveSolar3DRoutePreview) {
    delete window.receiveSolar3DRoutePreview
  }
})

function handleSolarPreviewMessage (event) {
  if (event.data?.type !== 'SOLAR_3D_ROUTE_PREVIEW') return
  applySolarPreviewPayload(event.data.payload)
}

function clearLocalSolarPreviewState () {
  exitCameraEditMode()
  clearEntities()
  PointsData.value = []
  removeRouteEntity()
  selectedEntity = null
  solarPreview.value = null
  selected_wayline.value = ''
  wayline_list.value = [{
    label: '等待光伏三维预览数据',
    value: ''
  }]
  sessionStorage.removeItem('solar3dRoutePreview')
  sessionStorage.setItem('waypointsData', JSON.stringify([]))
  sessionStorage.removeItem('selectedWayline')
}

function clearSceneForPreviewLoad () {
  exitCameraEditMode()
  clearEntities()
  PointsData.value = []
  removeRouteEntity()
...
function loadSolarPreview (payload) {
  ...
  selected_wayline.value = payload.route_draft_id
  wayline_list.value = [{
    label: payload.route_name || payload.route_draft_id,
    value: payload.route_draft_id
  }]
  sessionStorage.setItem('selectedWayline', payload.route_draft_id)

  const tempWaypoints = payload.waypoints.map((point, index) => {
    const entity = drawPoints(cesium.viewer, { ...pointOptions, text: `航点${index + 1}` }, point.lon, point.lat, point.height)
    PointsData.value.push({
      id: entity.id,
      lon: Number(point.lon).toFixed(14),
      lat: Number(point.lat).toFixed(14),
      height: Number(point.height).toFixed(14)
    })
    return {
      point_name: entity.id,
      waypoint_id: point.waypoint_id,
      longitude: point.lon,
      latitude: point.lat,
      height: point.height,
      camera_params: {
        heading: point.aircraft_heading ?? point.camera?.yaw ?? point.cameras?.[0]?.yaw ?? 0,
        pitch: point.camera?.pitch ?? point.cameras?.[0]?.pitch ?? -80,
        roll: point.camera?.roll ?? point.cameras?.[0]?.roll ?? 0,
        focalLength: point.camera?.focal_length_35mm ?? point.cameras?.[0]?.focal_length_35mm ?? 75
      },
      visible_focal_length_35mm: point.cameras?.find(camera => camera.image_format === 'visable')?.focal_length_35mm ?? point.camera?.focal_length_35mm ?? 75,
      ir_focal_length_35mm: point.cameras?.find(camera => camera.image_format === 'ir')?.focal_length_35mm ?? 52
    }
  })
...
const pointOptions = {
  ...
  paddingTo: 6, // 图标距离文字背景
  textPadding: 14, // 文字左右间距
  textBcgWidth: 200, // 文字背景宽度
  textBcgHeight: 36, // 文字背景高度
  iconWidth: 34, // 图标宽度
  iconHeight: 34, // 图标高度
...
function setupClickEvent () {
  ...
  handlePoint.setInputAction(function (event) {
    ...
    if (Cesium.defined(pickedObject) && isWaypointEntity(pickedObject.id)) {
      selectedEntity = pickedObject.id
      previousPosition = selectedEntity.position.getValue(Cesium.JulianDate.now())
      enterCameraEditModeForSelected()
      return
    }
  ...
  handlePoint.setInputAction(function (movement) {
    const pickedObject = cesium.viewer.scene.pick(movement.endPosition)
    cesium.viewer.scene.canvas.style.cursor = Cesium.defined(pickedObject) && isWaypointEntity(pickedObject.id) ? 'pointer' : 'default'
  }, Cesium.ScreenSpaceEventType.MOUSE_MOVE)
}

function isWaypointEntity (entity) {
  if (!entity?.id) return false
  if (PointsData.value.some(item => item.id === entity.id)) return true

  try {
    const waypointsInStorage = JSON.parse(sessionStorage.getItem('waypointsData')) || []
    return waypointsInStorage.some(wp => wp.point_name === entity.id)
  } catch {
    return false
  }
}
...
function movePoint (val) {
  ...
  updateStoredWaypointPosition(selectedEntity.id, newCartographic)
  updateRoutePolylineFromTemp()
...
function updateStoredWaypointPosition (pointName, cartographic) {
  try {
    const waypointsInStorage = JSON.parse(sessionStorage.getItem('waypointsData')) || []
    const waypoint = waypointsInStorage.find(wp => wp.point_name === pointName)
    if (!waypoint) return

    waypoint.longitude = Cesium.Math.toDegrees(cartographic.longitude).toFixed(14)
    waypoint.latitude = Cesium.Math.toDegrees(cartographic.latitude).toFixed(14)
    waypoint.height = cartographic.height.toFixed(14)
    sessionStorage.setItem('waypointsData', JSON.stringify(waypointsInStorage))
  } catch (error) {
    console.warn('同步移动后的航点位置失败:', error)
  }
}
...
function enterCameraEditModeForSelected () {
  if (!selectedEntity) return
  ...
  applyStoredCameraParamsToEditor()
  const currentPosition = selectedEntity.position.getValue(Cesium.JulianDate.now())
  ...
  childMap.value.setCamera(currentPosition, Math.round(value1.value), Math.round(value2.value), Math.round(value3.value), cameraParams.value.equivalentFocalLength)
...
function getStoredWaypointForSelected () {
  if (!selectedEntity) return null
  try {
    const waypointsInStorage = JSON.parse(sessionStorage.getItem('waypointsData')) || []
    return waypointsInStorage.find(wp => wp.point_name === selectedEntity.id) || null
  } catch {
    return null
  }
}

function applyStoredCameraParamsToEditor () {
  const waypoint = getStoredWaypointForSelected()
  const params = waypoint?.camera_params
  if (!params) return

  value1.value = Number(params.heading ?? value1.value)
  value2.value = Number(params.pitch ?? value2.value)
  value3.value = Number(params.roll ?? value3.value)
  if (params.focalLength != null) {
    cameraParams.value.equivalentFocalLength = Number(params.focalLength)
    cameraParams.value.fov = equivalentFocalLengthToFov(Number(params.focalLength))
  }
}

function equivalentFocalLengthToFov (equivalentFocalLength) {
  if (!equivalentFocalLength || equivalentFocalLength <= 0) return cameraParams.value.fov

  const sensorHeightMM = 7.0
  const sensorDiagonalMM = 9.6
  const actualFocalLength = equivalentFocalLength / (43.3 / sensorDiagonalMM)
  return 2 * Math.atan(sensorHeightMM / (2 * actualFocalLength))
}
...
function handleConfirm () {
  ...
  const originalWaypoint = waypointsInStorage.find(wp => wp.point_name === selectedId)
  const selectedPosition = selectedEntity.position.getValue(Cesium.JulianDate.now())
  const selectedCartographic = Cesium.Cartographic.fromCartesian(selectedPosition)
  const selectedLongitude = +Cesium.Math.toDegrees(selectedCartographic.longitude).toFixed(14)
  const selectedLatitude = +Cesium.Math.toDegrees(selectedCartographic.latitude).toFixed(14)
  const selectedHeight = +selectedCartographic.height.toFixed(14)

  // 获取当前相机参数（从最新场景中读取）
  const newWaypointData = {
    point_name: selectedId, // 用实体 ID 作为唯一标识
    waypoint_id: originalWaypoint?.waypoint_id,
    longitude: selectedLongitude,
    latitude: selectedLatitude,
    height: selectedHeight,
    capture_mode: captureMode.value, //
    speed: waypointSpeed.value,
    visible_focal_length_35mm: cameraParams.value.equivalentFocalLength,
    ir_focal_length_35mm: originalWaypoint?.ir_focal_length_35mm ?? 52,
    camera_params: {
...
async function handlegenerate () {
  ...
  isSolarRouteSubmitting.value = true
  const loadingInstance = ElLoading.service({
    lock: true,
    text: '正在回传光伏三维航线编辑结果，请稍候...',
    background: 'rgba(0, 0, 0, 0.25)'
  })
  try {
    if (!solarPreview.value?.route_draft_id) {
      const cachedPayload = readSolar3DPreviewPayload()
      if (cachedPayload?.route_draft_id) {
        solarPreview.value = cachedPayload
      } else {
        const { payload: devPayload, version } = await fetchDevSolar3DPreview()
        if (devPayload?.route_draft_id) {
          applySolarPreviewPayload(devPayload, version)
        }
      }
    }

    if (!solarPreview.value?.route_draft_id) {
      ElMessage.error('缺少 route_draft_id，请先接收光伏三维预览数据')
      return
    }
    ...
    const payload = {
      ...
      waypoints: waypointsData.map(point => ({
        ...
        aircraft_heading: Number(point.camera_params?.heading ?? 0),
        pitch: Number(point.camera_params?.pitch ?? -80),
        visible_focal_length_35mm: Number(point.visible_focal_length_35mm ?? point.camera_params?.focalLength ?? 75),
        ir_focal_length_35mm: Number(point.ir_focal_length_35mm ?? 52)
      }))
    }

    const invalidWaypoint = payload.waypoints.find(point =>
      !Number.isFinite(point.lon) ||
      !Number.isFinite(point.lat) ||
      !Number.isFinite(point.height) ||
      !Number.isFinite(point.aircraft_heading) ||
      !Number.isFinite(point.pitch) ||
      !Number.isFinite(point.visible_focal_length_35mm) ||
      !Number.isFinite(point.ir_focal_length_35mm)
    )
    if (invalidWaypoint) {
      console.error('光伏三维航线回传存在非法航点:', invalidWaypoint, waypointsData)
      ElMessage.error('回传失败：存在非法航点参数，请重新点击航点确认')
      return
    }

    console.log('准备回传光伏三维航线编辑结果:', payload)
...
function updateRoutePolylineFromTemp () {
  ...
  createOrUpdateGlowRoute(positions)
}

function createOrUpdateGlowRoute (positions) {
  removeRouteEntity()

  const routeCollection = new Cesium.PolylineCollection()
  cesium.viewer.scene.primitives.add(routeCollection)

  for (let index = 0; index < positions.length - 1; index += 1) {
    const segmentPositions = [positions[index], positions[index + 1]]
    routeCollection.add({
      positions: segmentPositions,
      width: 8,
      material: Cesium.Material.fromType(Cesium.Material.PolylineGlowType, {
        color: Cesium.Color.fromCssColorString('#00F5FF').withAlpha(0.28),
        glowPower: 0.28,
        taperPower: 0.0
      })
    })
    routeCollection.add({
      ...
      material: Cesium.Material.fromType(Cesium.Material.ColorType, {
        color: Cesium.Color.fromCssColorString('#8FFFFF').withAlpha(0.95)
      })
    })
  }

  routeEntity = cesium.viewer.entities.add({
    name: 'solar-wayline-glow',
    show: false
  })
  routeEntity._routeCollection = routeCollection
}

function removeRouteEntity () {
  if (!routeEntity) return

  if (routeEntity._routeCollection) {
    cesium.viewer.scene.primitives.remove(routeEntity._routeCollection)
  }
