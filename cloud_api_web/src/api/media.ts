...
export async function getFlyTaskResultApi (data): Promise<IWorkspaceResponse<{}>> {
  const url = `${HTTP_PREFIX}/files/getMediaFileByJobId?job_id=${data.job_id}&workspace_id=${data.workspace_id}&page=${data.page}&pageSize=${data.pageSize}&fileName=${data.fileName}`
  const result = await request.get(url)
  return result.data
}
