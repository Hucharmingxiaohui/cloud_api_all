...
export function useGMapCover () {
  ...
  // 只更新航点的标题
  function updatePinElementTitle (id:string, name: string) {
    const elements = getElementFromMap(id)
    if (elements && elements.length > 0) {
      const element = elements[0]
      element.setTitle(name)
      const content = `<div style="position: relative; display: inline-block;">
            <div style="position: absolute; bottom: 100%; left: 50%; transform: translateX(-50%); font-size: 13px; color: #fff; background-color: rgba(34,136,79,0.3); padding: 2px 5px; border-radius: 3px; white-space: nowrap;">
              ${name}
          </div>
          <img src="${pin2d8cf0}" style="width: 24px; height: 24px;"/>
      </div>`
      element.setContent(content)
      // element._opts.content =content
      // element._originOpts.content =content
    }
    const elements1 = getElementFromMap(id)
  }
