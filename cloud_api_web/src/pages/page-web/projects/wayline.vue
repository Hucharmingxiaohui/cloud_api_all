<template>
  <div class="project-wayline-wrapper">
    ...
    <div style="height: 50px; line-height: 50px; border-bottom: 1px solid #4f4f4f; font-weight: 450;">
     ...
     <!-- 搜索 -->
     <div style="height: 40px; line-height: 40px; border-bottom: 1px solid #4f4f4f; font-weight: 450;">
        <a-input style="background: black; width: 230px;margin-left: 10px;" v-model:value="searchValue"  placeholder="请输入文件名称" class="home-ant-input">
          <template #suffix >
            <a-tooltip title="搜索" >
               <a @click="SearchWayline">  <SearchOutlined style="color: rgba(255, 255, 255, 1)" /></a>
            </a-tooltip>
          </template>
        </a-input>
    </div>
    ...
    <div :style="{ height : height + 'px'}" class="scrollbar">
      <!-- //遍历所有航线 -->
      <div id="data" style="height: 500px" class=" uranus-scrollbar" v-if="waylinesData.data.length !== 0" @scroll="onScroll">
      ...
      <a-modal v-model:visible="deleteTip" width="450px" :closable="false" :maskClosable="false" centered :okButtonProps="{ danger: true }" @ok="deleteWayline">
          <p class="pt10 pl20" style="height: 50px;">航线文件一旦删除就无法恢复,是否继续？</p>
          <template #title>
